...
class Net(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        return out
#%%
data_step_list = [1, 2, 4]

n_pde_list = [1e03, 1e04, 1e05]

for data_step in data_step_list:
    for n_pde in n_pde_list:
        

        print(f"datastep = {data_step}, n_pde = {n_pde}")
        "PARAMETERS"
        max_iters = 20 #Max iterations
        ...
        # data_step = 1 #20 bilder
        # data_step = 2 #10 bilder
        # data_step = 4 #5 bilder
        
        
        
        script_dir = os.path.dirname(__file__)
        results_dir = os.path.join(script_dir, 'Results/', "learning_rates/", f"{20/data_step}_images_n_pde_{n_pde}/")
        ...
        plt.legend()
        plt.savefig(results_dir+"lossplot.png")
        plt.close()
        ...
        plt.ylabel("D")
        plt.xlabel("Iteration")
        plt.savefig(results_dir + "Dplot.png")
        plt.close()
        
        
        losses_numpy = np.array(losses)
        D_numpy = np.array(D_during_train)
        pde_numpy = pdeloss.numpy()
        data_numpy = dloss.numpy()
        
        
        np.save(results_dir+"D_array", D_numpy)
        np.save(results_dir+"pde_loss_array", pde_numpy)        
        np.save(results_dir+"data_loss_array", data_numpy)        
        #%%
        ...
        for i,t in enumerate(ts):
            ...
            if i % 1 == 0:
                ...
                plt.colorbar()
                # plt.savefig(results_dir+f"{i}_plot_data.png")
                plt.savefig(results_dir+f"{i}_plot_data.png")
                plt.figure(dpi=500)
                ...
                plt.colorbar()
                plt.savefig(results_dir+f"{i}_plot_nn.png")
                plt.close()
                if using_gpu == True:
                    xyt = xyt.to(device)
